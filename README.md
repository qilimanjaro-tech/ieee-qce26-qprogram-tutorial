...
QProgram depends only on NumPy and xarray, with Matplotlib as an optional `viz` extra for waveform and result plotting, and the tutorial adds nothing to that. [`setup/README.md`](setup/README.md) covers the install, whether you use pip or uv locally or run the notebooks on Google Colab, and the first two cells of `notebooks/01_introduction.ipynb` are the pass/fail check that your environment is ready.
